...
class AspirationProvider extends ChangeNotifier {
  ...
  // Delete Aspiration (Admin)
  Future<bool> deleteAspiration(String token, String aspirationId) async {
    _errorMessage = null;
    try {
      await ApiService.deleteAspiration(token, aspirationId);
      _aspirations.removeWhere((a) => a.id == aspirationId);
      _commentsByAspirationId.remove(aspirationId);
      notifyListeners();
      return true;
    } catch (e) {
      _errorMessage = e.toString().replaceFirst('Exception: ', '');
