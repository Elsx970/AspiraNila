...
class _AspirationDetailScreenState extends State<AspirationDetailScreen> {
  ...
  void _confirmDelete(String token, AspirationProvider provider, Aspiration asp) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Hapus Aspirasi'),
        content: const Text('Apakah Anda yakin ingin menghapus aspirasi ini secara permanen?'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              final success = await provider.deleteAspiration(token, asp.id);
              if (success && mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Aspirasi berhasil dihapus'), backgroundColor: AppTheme.accentColor),
                );
                Navigator.pop(context); // Return to list screen
              } else if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(provider.errorMessage ?? 'Gagal menghapus.'), backgroundColor: AppTheme.dangerColor),
                );
              }
            },
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildModerationButton(
                  label: 'Hapus Aspirasi Secara Permanen',
                  icon: Icons.delete_outline,
                  color: AppTheme.dangerColor,
                  isActive: false,
                  onPressed: () => _confirmDelete(token, provider, asp),
                ),
              ),
            ],
          ),
